...
lock = RLock()
url = 'http://httpbin.org/ip'  # 'http://www.ipdizhichaxun.com/'   #  http://tool.pc360.net/ip/
...
def checkproxy(proxy_ip, proxy_type):
	...
	t1 = datetime.now()
	try:
		# rq = urllib.request.urlopen(rq, timeout=3)
		# context = rq.read()
		# text = etree.HTML(context)
		ip = load(urllib.request.urlopen(rq))['origin']
		t2 = datetime.now()
...
def main():
	try:
		...
		my_ip = load(urllib.request.urlopen(rq))['origin']
